...
// Internal only — used by the styled background below. Knip flagged the
// export as unused, so it's kept module-local now.
...
export function Sidebar({ mobileOpen, onMobileClose }: { mobileOpen?: boolean; onMobileClose?: () => void }) {
  ...
  useEffect(() => {
    ...
    const fetchPendingCount = async () => {
      ...
      if (error) {
        console.error("[sidebar] pending count error:", error);
        return;
      }
