...
export function Sidebar({ mobileOpen, onMobileClose }: { mobileOpen?: boolean; onMobileClose?: () => void }) {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "patients", filter: `linked_doctor_id=eq.${user.id}` }, fetchPendingCount)
      .subscribe();
    return () => { supabase.removeChannel(channel); };
  }, [user]);

  const navItems = [
    { labelKey: "dash_sidebar_dashboard" as const, href: "/dashboard", icon: Home, badge: null, badgeIsCount: false },
    { labelKey: "dash_sidebar_add_patient" as const, href: "/dashboard/add-patient", icon: UserPlus, badge: "AI", badgeIsCount: false },
    { labelKey: "dash_sidebar_ready_for_diagnosis" as const, href: "/dashboard/ready-for-diagnosis", icon: Stethoscope, badge: pendingCount > 0 ? String(pendingCount) : null, badgeIsCount: true },
    { labelKey: "dash_sidebar_register" as const, href: "/dashboard/register", icon: Table2, badge: null, badgeIsCount: false },
    { labelKey: "dash_sidebar_patients" as const, href: "/dashboard/patients", icon: Users, badge: null, badgeIsCount: false },
    { labelKey: "dash_sidebar_prescriptions" as const, href: "/dashboard/prescriptions", icon: Pill, badge: null, badgeIsCount: false },
    { labelKey: "dash_sidebar_appointments" as const, href: "/dashboard/appointments", icon: Calendar, badge: null, badgeIsCount: false },
    { labelKey: "dash_sidebar_analytics" as const, href: "/dashboard/analytics", icon: BarChart3, badge: null, badgeIsCount: false },
    { labelKey: "dash_sidebar_cases" as const, href: "/dashboard/cases", icon: FileText, badge: null, badgeIsCount: false },
    { labelKey: "dash_sidebar_settings" as const, href: "/dashboard/settings", icon: Settings, badge: null, badgeIsCount: false },
  ];
                    ...
                    {item.badge && (
                      item.badgeIsCount ? (
                        <span
                          ...
                          style={{ background: "#b8936a", color: "#fff", fontSize: "11px" }}
                        >
                          {item.badge}
                        </span>
                      ) : (
